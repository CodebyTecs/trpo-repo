...
def load_openblas(path_from_user):
    ...
    raise RuntimeError("Не нашёл OpenBLAS. Укажи путь через --lib")
...
def make_buffers(dtype):
    ...
    raise ValueError("dtype должен быть s/d/c/z")
...
def main():
    ...
    parser.add_argument("--lib", default=None, help="Путь к libopenblas (например /usr/lib/libopenblas.so)")
    ...
    try:
        ...
        print("[FAIL] Не удалось загрузить OpenBLAS:", e)
        return 2
    ...
    if all_pass:
        print("\n[OK] Все Level 3 функции найдены и вызваны без ошибок")
        return 0

    print("\n[FAIL] Есть ошибки (symbol not found или исключение при вызове)")
    return 1
